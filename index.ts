...
/*
  ...
  subject$.next('Hello from subejct')


  let source$ = new Observable(subserver =>{
      subserver.next('Greetings!');
  })

  source$.subscribe(subject$);

  var subjectNumber$ = new Subject<number>();
  subjectNumber$.subscribe(value => console.log('Observer a : '+ value));
  subjectNumber$.subscribe(value => console.log('Observer b : '+ value));

  console.clear();
  subjectNumber$.next(1);
  subjectNumber$.next(2);
  */

  let source$ = interval(1000).pipe(take(4));

  source$.subscribe(value => console.log('Obserable 1 : '+ value))

  setTimeout(()=>{
    source$.subscribe(value => console.log('Obserable 2 : '+ value))
},1000)

  setTimeout(() =>{
    source$.subscribe(value => console.log('Obserable 3 : '+ value))
